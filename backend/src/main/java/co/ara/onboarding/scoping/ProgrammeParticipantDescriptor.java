...
/**
 * A programme's participant rows have no ownership of their own; they inherit
 * scope from the programme they belong to, the same viaCase shape
 ...
 * "programme_participant" resource type and DescriptorRegistry.validate() never
 * asks for one, but AuthorizedQuery.findAll/getById dispatch by ENTITY TYPE, so
 * ProgrammeService reading these rows under programme.view/programme.manage would
 * otherwise hit DescriptorRegistry.forEntity(ProgrammeParticipant.class) with
 * nothing registered -- the exact trap CaseParticipantDescriptor and
 * CaseAttributeValueDescriptor exist to close.
 *
 * assignedScope resolves through programme_participant on the actor, exactly like
 * ProgrammeDescriptor's own -- it must NOT reach into onboarding_case either.
 */
@Component
public class ProgrammeParticipantDescriptor implements ResourceAuthorizationDescriptor<ProgrammeParticipant> {

    @Override public String resourceType() { return "programme_participant"; }

    @Override public Class<ProgrammeParticipant> entityType() { return ProgrammeParticipant.class; }
    ...
    @Override public Specification<ProgrammeParticipant> departmentScope(AuthContext ctx) {
        return viaProgramme((root, query, cb, p) -> ctx.departmentId() == null
                ? cb.disjunction()
                : cb.equal(p.get("owningDepartmentId"), ctx.departmentId()));
    }

    @Override public Specification<ProgrammeParticipant> teamScope(AuthContext ctx) {
        return viaProgramme((root, query, cb, p) -> ctx.teamIds().isEmpty()
                ? cb.disjunction()
                : p.get("owningTeamId").in(ctx.teamIds()));
    }

    @Override public Specification<ProgrammeParticipant> assignedScope(AuthContext ctx) {
     ...
     * a participant row cannot be reached through a programme in another tenant.
     */
    private Specification<ProgrammeParticipant> viaProgramme(ProgrammeCondition condition) {
    ...
    private interface ProgrammeCondition {
        Predicate build(Root<ProgrammeParticipant> root, CriteriaQuery<?> query,
