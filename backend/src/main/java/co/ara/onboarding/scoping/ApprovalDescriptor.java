...
/**
 * Approvals have no ownership of their own; they inherit scope from the case they
 * belong to. Every scope therefore resolves through a subquery over Case rather
 * than over approval's own columns -- the same viaCustomer shape
 * CustomerContactDescriptor uses, walking up to onboarding_case instead.
 */
@Component
public class ApprovalDescriptor implements ResourceAuthorizationDescriptor<Approval> {

    @Override public String resourceType() { return "approval"; }

    @Override public Class<Approval> entityType() { return Approval.class; }
    ...
    @Override public Specification<Approval> departmentScope(AuthContext ctx) {
        return viaCase((root, query, cb, c) -> ctx.departmentId() == null
                ? cb.disjunction()
                : cb.equal(c.get("owningDepartmentId"), ctx.departmentId()));
    }

    @Override public Specification<Approval> teamScope(AuthContext ctx) {
        return viaCase((root, query, cb, c) -> ctx.teamIds().isEmpty()
                ? cb.disjunction()
                : c.get("owningTeamId").in(ctx.teamIds()));
    }

    @Override public Specification<Approval> assignedScope(AuthContext ctx) {
     ...
     * them. RLS still applies to the subquery's own table, so an approval cannot be
     * reached through a case in another tenant.
     */
    private Specification<Approval> viaCase(CaseCondition condition) {
    ...
    private interface CaseCondition {
        Predicate build(Root<Approval> root, CriteriaQuery<?> query,
