...
/**
 * Comments have no ownership of their own; every scope inherits from the case
 * they are attached to. case_id is NOT NULL on every comment -- including one
 * on a task -- specifically so this can resolve identically to CaseDescriptor
 * (design spec 6.2), the same viaCase shape MilestoneDescriptor and
 * CaseAttributeValueDescriptor use.
 */
@Component
public class CommentDescriptor implements ResourceAuthorizationDescriptor<Comment> {

    @Override public String resourceType() { return "comment"; }

    @Override public Class<Comment> entityType() { return Comment.class; }
    ...
    @Override public Specification<Comment> departmentScope(AuthContext ctx) {
        return viaCase((root, query, cb, c) -> ctx.departmentId() == null
                ? cb.disjunction()
                : cb.equal(c.get("owningDepartmentId"), ctx.departmentId()));
    }

    @Override public Specification<Comment> teamScope(AuthContext ctx) {
        return viaCase((root, query, cb, c) -> ctx.teamIds().isEmpty()
                ? cb.disjunction()
                : c.get("owningTeamId").in(ctx.teamIds()));
    }

    @Override public Specification<Comment> assignedScope(AuthContext ctx) {
     ...
     * against them. RLS still applies to the subquery's own table, so a comment
     * cannot be reached through a case in another tenant.
     */
    private Specification<Comment> viaCase(CaseCondition condition) {
    ...
    private interface CaseCondition {
        Predicate build(Root<Comment> root, CriteriaQuery<?> query,
